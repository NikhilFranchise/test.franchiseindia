...
@section('content')
    ...
    <div class="search-results container-fluid">
        <div class="search-result-inner">
            ...
            <a href="{{ route('tag.create', ['lang' => $lang]) }}" class="greens float-right btn btn-md btn-success">
                <i class="fa fa-plus-circle"></i>{{ ' Add New ' . $language . ' Tag' }}
            </a>

            <form action="{{ route('tag.list', ['lang' => $lang]) }}" method="get">
                ...
                <a href="{{ route('tag.list', ['lang' => $lang]) }}" class="btn btn-secondary"
        ...
        <div class="row-fluid">
            <div class="span12">
                <ul class="nav nav-tabs">
                    <li @if (url()->current() == route('tag.list', ['lang' => 'en'])) class="active" @endif><a
                            href="{{ route('tag.list', ['lang' => 'en']) }}">English Tags List</a></li>
                    <li @if (url()->current() == route('tag.list', ['lang' => 'hi'])) class="active" @endif><a
                            href="{{ route('tag.list', ['lang' => 'hi']) }}">Hindi Tags List</a></li>
                </ul>
            ...
            $('.deletetag').on('click', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('delete.tag', ['lang' => $lang, 'id' => '__ID__']) }}"
