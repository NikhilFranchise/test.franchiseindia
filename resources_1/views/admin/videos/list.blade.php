...
<div id="content-header">
    <div id="breadcrumb">
        ...
        <a href="{{ route('video.list', ['lang' => $lang]) }}" title="Go to Manage Videos" class="tip-bottom"><i class="fa fa-video"></i> Manage Videos</a>
        <a href="{{ url()->current() }}" class="current">{{ $language }} Videos</a>
    </div>
...
<div class="search-results container-fluid">
    <div class="search-result-inner">
        ...
        <a href="{{ route('video.create', ['lang' => $lang]) }}" class="greens float-right btn btn-md btn-success">
            <i class="fa fa-plus-circle"></i>{{ " Add New {$language} Video" }}
        </a>
        <form action="{{ route('video.list', ['lang' => $lang]) }}" method="get">
            ...
            <a href="{{ route('video.list', ['lang' => $lang]) }}"
    ...
    <div class="row-fluid">
        <div class="span12">
            <ul class="nav nav-tabs">
                <li @if (url()->current() == route('video.list', ['lang' => 'en'])) class="active" @endif><a
                        href="{{ route('video.list', ['lang' => 'en']) }}">English Videos</a></li>
                <li @if (url()->current() == route('video.list', ['lang' => 'hi'])) class="active" @endif><a
                        href="{{ route('video.list', ['lang' => 'hi']) }}">Hindi Videos</a></li>
            </ul>
                    ...
                    <table class="table table-bordered table-striped" id="videoTable">
                        ...
                        <tbody id="tablecontent">
                            ...
                            <tr class="gradeX">
                                ...
                                <td>
                                    <center>
                                        <button class="btn btn-warning" style="border-radius: 4px">
                                            <a
                                                href="{{ route('video.edit', ['id' => $data->sno, 'lang' => $lang]) }}">
                                ...
                                <td>
                                    <center>
                                        <button class="btn btn-danger deletevideo" style="border-radius: 4px"
                                            data-value="{{ $data->sno }}">
...
<script>
    $('.activestatus').on('change', function() {
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('video.status', ['lang' => $lang]) }}",
            data: {
                vId: id,
                status: status,
                _token: '{{ csrf_token() }}'
            ...
            success: function(data) {
                if (data.success) {
                    ...
                    Swal.fire({
                        ...
                        toast: true,
                        icon: 'success',
                        title: `Video status changed to ` + podStatus + `!`,
                        timer: 3000,
                        showConfirmButton: false,
                        timerProgressBar: true
                    });
    ...
    $('.deletevideo').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: "POST",
                    url: "{{ route('video.delete', ['lang' => $lang, 'id' => '__ID__']) }}"
                        .replace('__ID__', vId),
                    data: {
                        id: vId,
                        lang: lang,
