...
@section('content')
    <div class="container mostpopu arttilist settops" style="margin-top:10px">
        <div class="thanksmsg">
            <h2>Payment Cancelled</h2>
            @if(isset($payment))
                <p>Order ID: FIG-{{ $payment->pay_id }}</p>  <!-- Use pay_id -->
                <p>Name: {{ $payment->cname }}</p>
                <p>Amount: {{ $payment->camount }} {{ $payment->currency }}</p>
                {{-- <p>Mobile: {{ $payment->cphone }}</p>
                <p>Email: {{ $payment->cemail }}</p> --}}
            @else
