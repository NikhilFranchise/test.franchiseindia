<!-- Education startup start here -->

<div class="bor-radius backwhite pad20 ovfl mart20">
    <div class="ri-headingRt2">
        <h2><span><div>Education Startups</div></span></h2>
        <div class="ri-c-more"><a href="{{ Config('constants.MainDomain') }}/content/Education-Startups">More >></a></div>
    </div>
    <ul class="row ri-img-txt-list">
        @foreach( $startupArticles as $article )
            @php
        ...
        <li class="col-xs-12 col-sm-4 col-md-4">
            <ul class="ri-list-txt">
                ...
                @if( $loop->index > 1 )
                    <li>
                        <a href="{{ Config('constants.MainDomain') }}/education/{{$article['slug']}}.{{$article['content_id']}}">
                            {{$article['homeTitle']}}
                            @if($article['homeTitle']=='')
                                {{implode(' ', array_slice(explode(' ', substr(strip_tags($article['title']),0,100)), 0, 10))}}
                            @endif
