       ...
       <div class="section">
          ...
          <table>
             ...
             <tbody>
                ...
                <tr>
                   <td data-label="Types of Channels">{{($tradeData->channel_type == 5)? 'Distributor' : 'Dealer' }}</td>
    ...
    <div class="row expansion">
       <div class="tab-sec-topics">
        ...
        //   $keys = array_keys(array_column($stateList, 'region'), 'North');
            $keys = array_keys($stateListArray, 'North');
          ...
          @php
          $stateListArray = Arr::get(json_decode($stateList, true), 'region', []);
          $keys = array_keys($stateListArray, 'South');
        //   $keys = array_keys(array_column($stateList, 'region'), 'South');
          ...
          @php
           $stateListArray = Arr::get(json_decode($stateList, true), 'region', []);
            $keys = array_keys($stateListArray, 'East');
        //   $keys = array_keys(array_column($stateList, 'region'), 'East');
          ...
          @php
          $stateListArray = Arr::get(json_decode($stateList, true), 'region', []);
          $keys = array_keys($stateListArray, 'West');
          $westStates = '';
          foreach ($keys as $val) {
          ...
          @php
          $stateListArray = Arr::get(json_decode($stateList, true), 'region', []);
          $keys = array_keys($stateListArray, 'Center');
          $centerStates = '';
          foreach ($keys as $val) {
          ...
          @php
          $stateListArray = Arr::get(json_decode($stateList, true), 'region', []);
          $keys = array_keys($stateListArray, 'UT');
          $utStates = '';
          foreach ($keys as $val) {
