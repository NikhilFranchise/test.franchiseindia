...
<div class="sidearce bor-radius backwhite pad20 marfix">
    ...
    @foreach($newsArticles as $article)
        ...
        @if($i==1)
            ...
            <div class="rigtimg"><img src="https://www.franchiseindia.com/{{$article['image']}}" alt="{{$article['homeTitle']}}"></div>
        @endif
        @if($i!=1)
            <div class="smallatcmostright">
                ...
                <div class="subartbtmtext">
                    ...
                    @endif
                    @if($site == 'content')
                        <a href="{{ Config('constants.NewsDomain') }}/{{$article['slug']}}-{{$article['news_id']}}">
                            @if($article['homeTitle']=='')
                                {{implode(' ', array_slice(explode(' ', substr(strip_tags($article['title']),0,100)), 0, 10))}}
                            @endif
                            {{$article['homeTitle']}}
