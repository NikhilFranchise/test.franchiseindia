...
@php
    $ogimage = !empty($prevArticle->image)
        ? \App\Http\Controllers\InsightsController::createimgurl($prevArticle->image)
    ...
    $newsUrl =
        $baseUrl . strtolower($prevArticle->insight_type) . '/' . $prevArticle->slug . '.' . $prevArticle->news_id;
    $author_details = $prevArticle->author->first();
        ...
        <div class="col-md-8">
            ...
            <ul class="breadcrumb">
                ...
                @php
                    ...
                    $typeURL = $insightTypeMap[$prevArticle->insight_type] ?? null;
                @endphp
                <li class="breadcrumb-item">
                    <a href="{{ url("/insights/{$typeURL}") }}" class="tip-bottom">{{ $prevArticle->insight_type }}</a>
                </li>
                @foreach ($prevArticle->category as $category)
                    <li class="breadcrumb-item"><a href="{{ $baseUrl . $category->slug }}"
                            class="tip-bottom">{{ $category->catname }}</a></li>
                @endforeach
                @foreach ($prevArticle->Subcategory as $subcat)
                    <li class="breadcrumb-item"><a
                            href="{{ $baseUrl . $category->slug . '/' . $subcat->slug }}">{{ $subcat->subcat_name }}</a>
                    </li>
                @endforeach
                @desktop
                    <li class="breadcrumb-item">{!! html_entity_decode(\Illuminate\Support\Str::words($prevArticle->title, 8, ' ...'), ENT_QUOTES, 'UTF-8') !!}</li>
                @enddesktop
            </ul>
            <h2>{{ $prevArticle->title }}</h2>
            <div class="cont-top">
                <div class="article-features">
                    <div class="article-date">
                        ...
                        <div class="article-time">
                            ...
                            <span>
                                @if ($prevArticle->created_at >= $prevArticle->published_date)
                                    {{ date('M d, Y', strtotime($prevArticle->created_at)) }} /
                                @else
                                    {{ 'Last updated ' . date('M d, Y', strtotime($prevArticle->published_date)) }}
                                    /
                                @endif
                                <img src="{{ url('/insight-new/images/vicon.webp') }}" height="10" width="17"
                                    alt="Franchise Insights" class="img-fluid">
                                {{ $prevArticle->views }}
                                / {{ app\Http\Controllers\InsightsController::calculateReadTime($prevArticle) }}
                                Min Read
                            </span>
                        </div>
                    </div>
            ...
            <div class="content-main">
                <img src="{{ $ogimage }}" class="img-fluid" alt="{{ $prevArticle->title }}">
                ...
                {{-- ads for mobile & desktop --}}
                <div class="shortdes">{{ $prevArticle->shortDesc }}</div>
                <div class="articlecontent" data-article-id="{{ $prevArticle->news_id }}">
                    @php
