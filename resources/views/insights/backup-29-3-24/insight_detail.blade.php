...
<div class="maininnver">
    @php 
        ...
        if (!empty($author_details[0]['slug'])) { 
            $authorurl = Config('constants.MainDomain') . '/author/' . $author_details[0]['slug'] .'-' .$author_details[0]['author_id']; 
        }else{ 
            $slug = strtolower(str_replace(' ','-', $author_details[0]['title']));
            $authorurl = Config('constants.MainDomain') . '/author/' . $slug .'-' .$author_details[0]['author_id']; 
        } 
    @endphp
