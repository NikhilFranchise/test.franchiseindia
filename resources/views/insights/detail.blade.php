...
<div class="contentwrapper">
   ...
   <div class="container">
    ...
    foreach ($custom_data as $index => $cdata) {
        // Check if the paragraph contains a <table> tag
        $isTable = stripos($cdata, '<table') !== false;

        // Add ads above the table if this is a table paragraph
        if ($isTable && $adsInserted < $totalAdSlots) {
        ...
        // Add ads below the table if this is a table paragraph
        if ($isTable && $adsInserted < $totalAdSlots) {
            $adSlotId = $adSlots[$adsInserted % $totalAdSlots];
            $articleData[] = '<div class="inner-article-detail-desktop-ad">
                <div id="' . $adSlotId . '">
        ...
        // Insert ads at the usual interval, but only if it's not a table
        if (!$isTable && $adsInserted < $adInsertCount && ($index + 1) % $adInterval == 0) {
            $adSlotId = $adSlots[$adsInserted % $totalAdSlots];
            $articleData[] = '<div class="inner-article-detail-desktop-ad">
                <div id="' . $adSlotId . '">
                    <script>
