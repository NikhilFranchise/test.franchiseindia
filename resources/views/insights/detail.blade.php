         ...
         <div class="col-md-8">
            ...
            <div class="content-main">
               ...
               <div id="main-article-container">
                  ...
                  <div
                     style="display: flex; flex-wrap: wrap; gap: 15px; justify-content: center; padding: 20px;">
               ...
               <div id="next-articles-container"></div>
               <div id="loading" style="display: none;">Loading next article...</div>
            </div>
...
<script>
   let isLoading = false;
   let currentPage = 1;
   const initialArticleId = {{ $newsDetails->news_id }};
   const categoryId = {{ $newsDetails->category[0]->id }};
   const locale = "{{ app()->getLocale() }}";
   let loadedArticles = [initialArticleId];
   ...
   function loadNextArticle() {
       ...
       fetch(`/api/next-article?current=${initialArticleId}&category=${categoryId}&loaded=${loadedArticles.join(',')}&locale=${locale}`
               )
           .then(response => response.json())
           .then(data => {
               if (data.article) {
                   const container = document.getElementById('next-articles-container');
                   container.insertAdjacentHTML('beforeend', renderArticle(data));
                   loadedArticles.push(data.article.news_id);
                   currentPage++;
...
function renderArticle(data) {
    const tags = data.assocTags || [];
    const franchiseData = data.franchiseData || [];
    const article = data.article || {};
    const locale = data.locale || 'en';  // Default locale to 'en'

    // Ensure category and subcategory are arrays
    const categories = Array.isArray(article.cat_id) ? article.cat_id : [];
    const subcategories = Array.isArray(article.subcat_id) ? article.subcat_id : [];
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-8">
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item">
                                <a href="/insights" class="tip-bottom">Home</a>
                            </li>
                            <li class="breadcrumb-item">
                                <a href="/insights/${article.insight_type || 'general'}" class="tip-bottom">${article.insight_type || 'Insight'}</a>
                            </li>
                            ${categories.map(cat_id => `
                                <li class="breadcrumb-item">
                                    <a href="/${category.slug}" class="tip-bottom">${category.catname}</a>
                                </li>
                            `).join('')}
                            ${subcategories.map(subcat_id => `
                                <li class="breadcrumb-item">
                                    <a href="/${subcat.slug}" class="tip-bottom">${subcat.subcat_name}</a>
                                </li>
                            `).join('')}
                        </ul>
                        ...
                        <div class="cont-top">
                            <div class="article-features">
                                <div class="article-date">
                                    <div class="article-logo">
                                        <img src="${article.author_image || '/path/to/default/author_image.jpg'}" width="51" height="51" alt="${article.author_name || 'Author'}" loading="lazy" />
                                    </div>
                                    <div class="article-time">
                                        BY - <a href="${article.author_url || '#'}">${article.author_name || 'Author'}</a><br>
                                        <span>${article.author_designation || 'Designation'}</span>
                                        <span>${new Date(article.created_at).toLocaleDateString()} / 
                                            <img src="/insight-new/images/vicon.webp" height="10" width="17" alt="Franchise Insights" class="img-fluid" /> 
                                            ${article.views || 0} / 
                        ...
                        <div class="content-main">
                            <img src="${article.image || '/path/to/default/image.jpg'}" class="img-fluid" alt="${article.title || 'Untitled Article'}">
                            <!-- Short Description -->
                            <div class="shortdes">${article.shortDesc || 'No description available.'}</div>
                                ...
                                <div style="display: flex; flex-wrap: wrap; gap: 15px; justify-content: center; padding: 20px;">
                                    ...
                                    ${franchiseData.map(franchise => `
                                        <div style="background-color: #f9f9f9; border: 1px solid #ddd; padding: 15px 20px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1); text-align: center; min-width: 95px; max-width: 200px;">
                                            <a href="/brands/${franchise.profile_name}.${franchise.fran_detail_id}" target="_blank" style="text-decoration: none; color: #333; font-weight: bold; font-size: 16px;">
                                                ${franchise.company_name}
                                            </a>
                                        </div>
                                    `).join('')}
                            ...
                            <div class="tag-block">
                                <ul class="tag-list">
                                    ${tags.map(tag => `
                                        <li><a href="/insights/${locale}/tag/${tag.slug}">${tag.name}</a></li>
                                    `).join('')}
                                </ul>
                            </div>
                        </div>
   ...
   window.addEventListener('scroll', () => {
       ...
       const mainArticle = document.getElementById('main-article-container');
       const scrollPosition = window.innerHeight + window.scrollY;
       const articleBottom = mainArticle.offsetTop + mainArticle.offsetHeight;
   
       if (scrollPosition > articleBottom - 500) {
           loadNextArticle();
           isThrottled = true;
           setTimeout(() => {
               isThrottled = false
