...
<nav id="sidebar">
    ...
    <ul class="list-unstyled components border-bottom-1">
        ...
        @foreach($categoryArr as $key => $value)
            ...
            @endphp
            <li class="{{($i == 0)? 'active': ''}}">
                <a href="#{{$catClass}}" data-toggle="collapse" aria-expanded="false">{{Config('hindiConstants.CategoryArr.'.$key)}}
                        ...
                        <ul class="collapse list-unstyled" id="{{$subcatClass}}">
                            @foreach(Config('constants.subSubCategoryArr.'.$key1) as $key2 => $value2)
                                @if(in_array($key2, json_decode(\Illuminate\Support\Facades\Storage::getFacadeRoot()->get('ssc.json'), true)))
                                    <li><a target="_blank" href="/business-opportunities/{{Config('category.SeoSubSubCategoryArr.'.$key2)}}.ssc{{$key2}}">{{Config('hindiConstants.subSubCategoryArr.'.$key1.'.'.$key2)}}</a></li>
                                @endif
                            @endforeach
                        </ul>
                    </li>
        ...
        @endforeach
            ...
            <li>
                <a target="_blank" href="https://www.franchiseindia.com/business-opportunities/lowcost"> कम लागत वाले व्यवसाय के अवसर </a>
            </li>
