...
<head>
        <title>Franchisor Login -Franchise India </title><meta charset="UTF-8" />
    ...
    <body>
        <div id="loginbox">            
            <form id="loginform" class="form-vertical" action="{{Config::get('constants.MainDomain')}}/fibl/login" method="POST">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            @if (Session::has('loginFailed'))
                     <div class="alert alert-info">{{ Session::get('loginFailed') }}</div>
                  @endif
				 <div class="control-group normal_text"> <h3><img src="{{URL::asset('admin/img/logo.png')}}" alt="Logo" /></h3></div>
                <div class="control-group">
                    <div class="controls">
                        <div class="main_input_box">
                            <span class="add-on bg_lg"><i class="icon-user"> </i></span><input type="text" placeholder="FRANCHISOR ID" name="fid" required/>
                        </div>
                    </div>
                </div>
                <div class="control-group">
