...
@section('content')
    ...
    <div id="content-header">
        <div id="breadcrumb">
            <a href="{{ route('crreAdmin.dashboard', ['lang' => $lang]) }}" title="Go to Home" class="tip-bottom">
                <i class="fa fa-home"></i> Home</a>
            <a href="{{ route('crreAdmin.content.list', ['lang' => $lang]) }}" class="tip-bottom"
                title="Go to Manager Content">
                <i class="fa fa-newspaper"></i> Manage Content</a>
            <a href="{{ url()->current() }}" class="current">{{ $language }} Content</a>
        </div>
    ...
    <div class="search-results container-fluid">
        <div class="search-result-inner">
            ...
            <a href="{{ route('crreAdmin.content.create', ['lang' => $lang]) }}"
                class="greens float-right btn btn-md btn-success">
                <i class="fa fa-plus-circle"></i>{{ ' Add ' . $language . ' Content' }}
            </a>
            <form method="get" action="{{ route('crreAdmin.content.list', ['lang' => $lang]) }}">
                ...
                <a href="{{ route('crreAdmin.content.list', ['lang' => $lang]) }}" class="btn btn-secondary"
        ...
        <div class="row-fluid">
            <div class="span12">
                <ul class="nav nav-tabs">
                    <li @if (url()->current() == route('crreAdmin.content.list', ['lang' => 'en'])) class="active" @endif><a
                            href="{{ route('crreAdmin.content.list', ['lang' => 'en']) }}">English Content</a></li>
                    <li @if (url()->current() == route('crreAdmin.content.list', ['lang' => 'hi'])) class="active" @endif><a
                            href="{{ route('crreAdmin.content.list', ['lang' => 'hi']) }}">Hindi Content</a></li>
                </ul>
                        ...
                        <table class="table table-bordered table-striped" id="insightsTable">
                            <thead>
                                <tr>
                                    ...
                                    <th rowspan="2">Content Type/ (Views)</th>
            ...
            $('.deleteInsight').click(function() {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: "POST",
                            url: "{{ route('crreAdmin.content.delete', ['lang' => $lang, 'id' => '__ID__']) }}"
