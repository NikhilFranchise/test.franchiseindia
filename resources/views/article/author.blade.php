...
<ul class="nabva">
<li><a href="{{url('/newcontent')}}">Home</a></li>
<li>/</li>
                    ...
                    @foreach($authorArticles as $article)
                        @if($loop->index < 5)
                            @php
                                ...
                                $url    = Config('constants.MainDomain').'/newcontent/'.$art->slugify($article->content_title,'-').'.'.$article->content_id;
                                $kicker = Config('constants.MainDomain').'/newcontent/'.$art->slugify($article['kicker'],'-');
