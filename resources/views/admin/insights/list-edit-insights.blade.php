...
<div id="content">
    ...
    <div id="content-header">
        <div id="breadcrumb"> <a href="{{ url('admin/dashboard') }}" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="list-insights" class="tip-bottom">Insights</a> 
        <a href="" class="current">List-Insights</a> </div>
...
<div style="margin-top: 5%;float: right;" class="container-fluid">
                <form action="{{url('admin/list-insights')}}" method="get">
                    Search Keyword :  <input type="text" name="search"class="span7" placeholder="Enter Title or Insights Id to search"
                    ...
                    <div class="widget-content nopadding">
                        <table class="table table-bordered table-striped">
                            ...
                            @foreach ($data as $insights)
                                <tr class="gradeX">
                                    @php
                                        $url = '/insights/'. strtolower($insights->insight_type) .'/'.$insights->slug.'.'.$insights->news_id;
                                    @endphp
                                    <td>{{$insights->news_id}}</td>
                                    <td>{{$insights->title}}</td>
                                    <td>@if(empty($insights->insight_type) && ($insights->insight_type == 0)){{'No Insights Type '}}@else {{$insights->insight_type}} @endif </td>
                                    {{-- <td>{{$insights->category}}</td> --}}
                                    ...
                                    <td>
                                        <center>
                                            <label class="switch">
                                                <input type="checkbox" value="{{$insights->news_id}}" class="activestate" {{($insights->status == 1) ? "checked" : ""}}>
                                                <span class="slider round" ></span>
                                            </label>
                                        </center>
                                    </td>
                                    <td><center><button class="btn btn-medium btn-warning" style="border-radius: 4px"><a href="edit-insights-view/{{$insights->news_id}}">Edit</a></button></center></td>
                                    ...
                                    <td><center><button class="btn btn-medium btn-danger deleteauthor" style="border-radius: 4px" data-value="{{$insights->news_id}}">Delete</button></center></td>
                                </tr>
                            @endforeach
                            </tbody>
                        </table>
