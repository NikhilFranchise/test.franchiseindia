...
.custpagin .pagination li a{ padding: 3px 7px; background-color: #41BEDD; color: #fff; border-radius: 6px;border:1px;}
  ...
  <div id="content-header">
    <div id="breadcrumb"> <a href="dashboard" title="Go to Home" class="tip-bottom"><i class="icon-home"></i> Home</a> <a href="#" class="tip-bottom">Magazine</a> <a href="#" class="current">List-Magazine</a> </div>
  ...
  <!--End-breadcrumbs-->
    ...
    <div class="container-fluid">
        ...
        <div class="span12">
          ...
          <div class="widget-box">
            <div class="widget-title"> <span class="icon"><i class="icon-th"></i></span>
              <h5>Magazine Data</h5>
              <div class="" style=" position: relative; left:650px; padding-top: 2px;"><label>Search: 
              <input type="text" id="searchmagazine" placeholder="enter keyword to search">
            </label></div>
            </div>
            <div class="widget-content nopadding">
              <table class="table table-bordered table-striped">
                ...
                @foreach($magazineData as $data)
                  <tr class="gradeX">
                    <td>{{$data->category_id}}</td>
                    <td><a href="list-magazine-articles/{{$data->category_id}}">{{$data->title}}</a></td>
                    <td>
                      <center>
                        <label class="switch">
                          @php
                            ...
                            if ( $data->status == 1)
                              $check="checked";
                          @endphp
                          <input type="checkbox" value="{{$data->category_id}}" {{$check}} class="activestate" >
                          <span class="slider round" ></span>
                        </label>
                      </center>
                    </td>
                    <td><center><a href="edit-magazine-view/{{$data->category_id}}"><button class="btn btn-medium btn-warning" style="border-radius: 4px">Edit</button></a></center></td>
                    <td><center><button class="btn btn-medium btn-danger deleteauthor" style="border-radius: 4px" data-value="{{$data->category_id}}">Delete</button></center></td>
                  </tr>
                  @endforeach
                  <!-- Modal confirm -->
      ...
      <div class="custpagin">{{ $magazineData->links() }}</div>
    </div>
  </div>
  <!--Footer-part-->
  ...
  <script src="{{URL::asset('admin/js/jquery.min.js')}}"></script>
    ...
    $('.activestate').click(function() {
      if(this.checked){
           ...
           $.ajax({
                type: "POST",
                url: '/updatemagazinestatus',
                data: {"magazine":id,"contentStatus":"1","_token" :"{{csrf_token()}}"},
            });
         }
      if(!(this.checked)){
          var id = this.value;
           $.ajax({
                type: "POST",
                url: '/updatemagazinestatus',
                data: {"magazine":id,"contentStatus":"0","_token" :"{{csrf_token()}}"},
            });
         }
      });
      ...
      $('.deleteauthor').on('click', function(e){
        ...
        confirmDialog(YOUR_MESSAGE_STRING_CONST, function() {
           $.ajax({
                type: "POST",
                url: '/deletemagazine',
                data: {"magazineId":x,"_token" :"{{csrf_token()}}"},
  ...
  <script type="text/javascript">
  $( "#searchmagazine" ).keyup(function() {
  var value = $("#searchmagazine").val();;
  $.ajax({
      type : 'get',
      url  : '{{URL::to('searchmagazine')}}',
      data : {'search':value},
      success:function(data){
