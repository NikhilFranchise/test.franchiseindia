...
public function get_country()
{
    try {
        ...
        $mapped = collect($countries['data'] ?? [])->map(function ($country) use ($codes) {

            $code = collect($codes['data'] ?? [])
                ->firstWhere('name', $country['name'] ?? null);

            return [
                'country_name' => $country['name'] ?? null,
                'phone_code'   => $code['dial_code'] ?? null,
...
public function getCities(Request $request)
{
    try {
        ...
        $query = '
        [out:json][timeout:25];
        area["name"="'.$request->country.'"]["boundary"="administrative"]->.country;
        area["name"="'.$request->state.'"]["boundary"="administrative"](area.country)->.state;
        (
        ...
        $cities = collect($response['elements'] ?? [])
            ->pluck('tags.name')
            ->filter()
            ->unique()
            ->values();
...
private function curlGet($url, $postData = null)
{
    $ch = curl_init($url);
