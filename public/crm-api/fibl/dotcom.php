...
$date1 = date('Y-m-d H:i:s', strtotime(' -6 minute'));

include 'getLeadBrandType.php';
include 'common.php';
$BxData = [];
$sqlQuery = "SELECT franchisor_id, name, lname, email, phone, city, state, address, investment, pincode, category, create_date FROM express_fran_insta_apply WHERE `create_date` >= '". $date1 ."' GROUP BY phone";
...
if($SelectCount > 0){
	while ($row = mysqli_fetch_assoc($queryData)) {
		...
		if($SelectCountTemp > 0){
			$SrowTemp = mysqli_fetch_assoc($queryDataTemp);
			$fibl_brands = getLeadTypeBx($SrowTemp['fibl_brands']);
			$lead_source = 4;		
