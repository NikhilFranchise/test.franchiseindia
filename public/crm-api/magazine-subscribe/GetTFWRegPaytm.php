...
include_once("../configpg/paytm_fi_config.php");    //HDFC settings page
include_once('../configpg/paytm_fi_encdec.php');
...
if($isValidChecksum == "TRUE") {
	...
	//	}
		...
		if ($dumpvalue['STATUS'] == "TXN_SUCCESS" && $dumpvalue['RESPCODE'] == 01){
			...
			if($dumpvalue['TXNAMOUNT'] == $resArr['amount'] || $dumpvalue['TXNAMOUNT'] == $resArr['amountfinal']){
			...
			//	echo "<b>Amount</b> " .$dumpvalue['TXNAMOUNT']. "<br/>";									
				...
				$query = mysqli_query($link,"UPDATE magazine_subscribe SET payment_status = 'Y', pgresponse = '".$order_status."', saltkey = '".$tracking_id."', comments = '".$bank_ref_no."' WHERE sub_sub_id = $txnidu");
		...
		$query = "UPDATE magazine_subscribe SET payment_status = 'F', pgresponse = '".$dumpvalue['RESPMSG']."', saltkey = '".$dumpvalue['BANKTXNID']."' WHERE sub_id = $txnidu";
		$result = mysqli_query($link,$query);		
			//header('Location:http://www.franchiseindiaevents.com/exclusive-events/thanks.php?message=PAYMENT Failed');
			//exit;				
		$txnid = base64_encode($txnidu);
		header('Location:https://master.franchiseindia.com/magazine-subscribe/FailedTRAN.php');				
		exit;
