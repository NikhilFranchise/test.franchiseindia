...
$MagCode = array(
      ...
      $urlCreateSubscription  = "https://www.zohoapis.com/billing/v1/subscriptions";
      $from = 0;
      $perPage = 180;
     ...
     // $obj_function          = new general_function();
    //  $rs_subscribeCount     = $obj_function->get_zoho_magazine_cid_count($from , $perPage); //
      $rs_subscribeCount     = mysqli_query($link,"SELECT `sub_id` FROM `magazine_subscribe` WHERE  `sub_id` > $startid AND payment_status = 'Y' LIMIT $from , $perPage");	  
      $subscribeCount	       = mysqli_num_rows($rs_subscribeCount);
    // Code to fetch data from magazine //
      //$pg_result 		         = $obj_function->get_zoho_magazine_cid_pagination($from , $perPage);
		$pg_result 		         = mysqli_query($link,"SELECT `name`, `email`, `company`, `address`, `country`, `state`, `city`, `pincode`, `telephone`, `magazine`, `magazine_offer`, `subscription`, `amount`, `amountfinal`, `cid`, `payment_mode`, `delivery_mode`, `order_type`, `campaign`, `designation`, `submitted_date`, `created_at`, `updated_at` FROM `magazine_subscribe` WHERE  `sub_id` > $startid AND payment_status = 'Y' ORDER BY created_at DESC LIMIT $from , $perPage  ");	  
      $num_of_record	       =  mysqli_num_rows($pg_result);
      //$fetch_details 	       = $obj_function->get_fetch_record($pg_result);
     // $fetch_details 	       = mysqli_fetch_assoc($pg_result);
	//  echo "<pre>";	  
	//print_r($fetch_details);
	
		while($fetch_details =mysqli_fetch_assoc($pg_result)){
   // for($i=0;$i<$num_of_record;$i++){
			//echo $fetch_details['email']."<br>";
    // api for check customer using email id
      // $url        = "https://subscriptions.zoho.com/api/v1/customers?email=".$fetch_details['email'];
       $url        = "https://www.zohoapis.com/billing/v1/customers?email=".$fetch_details['email'];
   ...
   // if customer not in zoho crm create new customer // 
    if(empty($customerID['customers'])){
		   $submitted_date = explode(" ",$fetch_details['submitted_date']);
           $key = $MagCode[trim($fetch_details['magazine'])];
           // if mag code found in array
            if(!empty($key)){
                    $dataSubscriptions = array(
                        "customer" => array(
                            "display_name"  =>$fetch_details['name'],
                            "salutation"    =>"",
                            "first_name"    =>$fetch_details['name'],
                            "last_name"     =>"",
                            "email"         =>$fetch_details['email'],
                            "company_name"  =>$fetch_details['company'],
                            "mobile"        =>$fetch_details['telephone'],
                        "billing_address" => array(
                            "street"  =>$fetch_details['address'],
                            "city"    =>$fetch_details['city'],
                            "state"   =>$fetch_details['state'],
                            "country" =>$fetch_details['country'],
                            "zip"     =>$fetch_details['pincode'],
                            "fax"     =>""
                        ),
                    ...
                    "plan"=>array(
                            "plan_code" =>$key,
                            "plan_description"=>$fetch_details['magazine'],
                            "price"           =>$fetch_details['amount'],
                            "quantity" =>'1'
                       ),
                    "create_backdated_invoice"=> true, 
                  );
					//print_r($dataSubscriptions);
                 ...
                 if(!empty($message)){ 
				 	//	echo "here";
                      ...
                      //   $urlpaymentCreate = "https://subscriptions.zoho.com/api/v1/payments";
                         $urlpaymentCreate = "https://www.zohoapis.com/billing/v1/payments";
				  ...
				  // end payment   
                        ...
                        echo "<br><strong>some problem in Subscriptions data1</strong><br>";
			             ...
			             $mag_code = explode("*", $fetch_details['magazine']);
				          for($j = 0; $j < count($mag_code); $j++){
                  //Mukesh
                    $submitted_date = explode(" ",$fetch_details['submitted_date']);
				            $product_id     = array_search(substr($mag_code[$j],0,1), $ProdCode);
					          $plan_code      = $MagCode[trim($mag_code[$j])];
                    $query          = "SELECT `pay_amount` FROM `shop_order_position` WHERE `order_id` = '".$fetch_details['orderid']."' AND `product_id` = '$product_id'";
       ...
       // if customer in zoho crm create Subscriptions // 
		    ...
		    $key = $MagCode[$fetch_details['magazine']];
        $submitted_date = explode(" ",$fetch_details['submitted_date']);
