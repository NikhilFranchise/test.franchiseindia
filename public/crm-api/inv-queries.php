...
if (!empty($date)) {
    $sql = "SELECT name, email, mobile, country, state, city, address, pincode, created_at FROM ask_investor where `created_at` >= '".$date."'  limit " . $offset . "," . $limit;
}else{
    $sql   = "SELECT name, email, mobile, country, state, city, address, pincode, created_at FROM ask_investor limit " . $offset . "," . $limit;
}
