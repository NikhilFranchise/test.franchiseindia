...
class AuthorController extends Controller
{
    ...
    {
        return view('crreAdmin.author.create');
    }
    ...
    public function registerAuthor(Request $request)
    {
        ...
        $request->validate([
            ...
            'email'      => 'required|email|unique:crre_admin_users,admin_email',
        ...
        $adminUser = CrreAdminUsers::create([
        ...
        // Create CrreAuthors linked to AdminUser
        CrreAuthors::create([
        ...
        return redirect()->route('crreAdmin.list.author')->with('success', 'Author (User) profile created.');
    }
    ...
    {
        $authorId = $request->id;
        $author    = CrreAuthors::query()->where('author_id', $authorId)->first();
        return view('crreAdmin.author.create', ['author' => $author]);
    }
    ...
    public function listAuthor(Request $request)
    {
        ...
        $authors->getCollection()->transform(function ($author) {
            $englishCount = CrreContentList::where('author_id', $author->author_id)
                ->whereNotIn('news_type', ['ir', 'ri'])
                ->where('status', 1)
                ->count();

            $hindiCount = CrreHindiContentList::where('author_id', $author->author_id)
                ->whereNotIn('news_type', ['ir', 'ri'])
                ->where('status', 1)
                ->count();
        ...
        return view('crreAdmin.author.list', compact('authors', 'totalRecords'));
    }
    ...
    public function updateAuthor(Request $request)
    {
        ...
        $author = CrreAuthors::with('admin')->findOrFail($request->author_id);
        $adminUser = $author->admin;

        // CASE 1: If author->admin_id is empty but same email exists in admin_users
        if (empty($author->admin_id)) {
            $existingAdmin = CrreAdminUsers::where('admin_email', $request->email)->first();
            if ($existingAdmin) {
        ...
        if ($request->profile == 1) {
            ...
            return redirect()->route('crreAdmin.dashboard')->with('success', 'Your Profile Updated.');
        }
        return redirect()->route('crreAdmin.list.author')->with('success', 'Author (User) profile updated.');
    }
    ...
    public function updateAuthorStatus(Request $request)
    {
        ...
        try {
            $validated = $request->validate([
                'authorId'     => 'required|exists:crre_authors,author_id',
                'authorstatus' => 'required|in:A,D', // A = Active, D = Deactivated
            ]);

            $author = CrreAuthors::find($validated['authorId']);
    ...
    public function deleteAuthor(Request $request)
    {
        ...
        $author = CrreAuthors::find($authorId);
        ...
        if ($author) {
            ...
            // Delete CrreAuthors entry
            $author->delete();
            return redirect()->route('crreAdmin.list.author')->with('success', 'Author (User) profile deleted successfully.');
        } else {
            return redirect()->route('crreAdmin.list.author')->with('error', 'Author not found.');
        }
    }
    ...
    public function authorProfile($id)
    {
        $authorId = $id;
        $author    = CrreAuthors::query()->where('author_id', $authorId)->first();
        return view('crreAdmin.author.profile', ['author' => $author]);
    }
}
