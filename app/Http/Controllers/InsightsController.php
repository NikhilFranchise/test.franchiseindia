    ...
    public function getNextArticle(Request $request)
{
    ...
    $loadedIds = explode(',', $request->input('loaded'));
    $locale = $request->input('locale');

    // Get the article with all the related data
    $article = InsightList::with(['category', 'Subcategory', 'author'])
    ...
    // Process content with ads (reuse your existing PHP logic)
    $processedContent = $this->processArticleContent($article->content);

    // Process franchise data
    $franchiseData = FranchisorBusinessDetail::where('article_id', $article->news_id) // Assuming there's a relationship between franchise and article
        ->get()
        ->map(function ($franchise) {
            return [
                'company_name' => $franchise->company_name,
                'profile_name' => $franchise->profile_name,
                'fran_detail_id' => $franchise->fran_detail_id,
            ];
        })
        ->toArray();

    // Process tags associated with the article
    $assocTags = ContentTagsAssigned::where('content_id', $article->news_id)
        ->where('content_type', 2)
        ->with('tag')
        ->get()
    ...
    return response()->json([
        'article' => [
            ...
            'author' => [
                ...
                'designation' => $article->author->designation,
                'author_url' => $article->author->author_url,
                'profile_image' => $article->author->profile->image ?? '', // Assuming profile image is in a related profile table
            ],
    ...
    private function processArticleContent($content)
    {
        // Reuse your existing content processing logic with ads
        $custom_data = explode("\r\n", $content);
        // ... rest of your ad insertion logic ...
        return implode("\r\n", $custom_data);
    }
}
