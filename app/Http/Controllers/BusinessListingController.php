...
if (!empty(request()->city)) {
    ...
    $cityAlias = [
        'Bengaluru' => ['Bengaluru', 'Bangalore', 'Bangalore Urban District'],
    ...
    // If alias exists → use alias; else use city
    $citySearch = $cityAlias[$city] ?? [$city];

    // Prevent NULL
    $stateNames = config('location.StateCity.' . $city) ?? null;

    // Safe fallback
    $cityArr = FranchisorLocState::query()
        ->whereIn('city', $citySearch)
        ->pluck('franchisor_id')
        ->toArray();

    if (empty($cityArr) && !empty($stateNames)) {
        $cityArr = FranchisorLocState::query()
            ->where('state', $stateNames)
            ->limit(50)
            ->pluck('franchisor_id')
    ...
    public function searchBusinessListing(Request $request)
    {
        ...
        */
        ...
        //Pankaj start for search data save in database
    ...
    public function getBusinessListing(Request $request)
    {
        ...
        //         ->where('end_date', '>=', date('Y-m-d H:i:s'))
        //         ->get()->pluck('franchisor_id');
        //     // regional Paid franchisors addition
        //     $extraFranchisors = collect([
        //         'FIHL515652',
        //         'FIHL324265',
        //         'FIHL483834',
        //         'FIHL349146',
        ...
        //     $sliderCheck = $sliderCheck->concat($extraFranchisors);
        //     // regional Paid franchisors addition
